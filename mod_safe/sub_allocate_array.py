    ...
    for typ in sorted(data.keys()):
        ...
        for knd in data[typ]:
            ...
            for dim in range(7):
                ...
                # Save source ...
                with open(f"sub_allocate_array/sub_allocate_{dim + 1:d}D_{knd}_{typ}_array.f90", mode = "wt", encoding = "utf-8") as fObj:
                    fObj.write(src)

    # Open output file ...
    with open("sub_allocate_array.tmp", mode = "wt", encoding = "utf-8") as fObj:
