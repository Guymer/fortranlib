!> @brief This subroutine allocates a 1D INT16 integer array.
!>
!> @param[inout] arr The 1D INT16 integer array to be allocated
!>
...
SUBROUTINE sub_allocate_1D_INT16_integer_array(arr, name, n1, debug)
    ...
    IF(debug)THEN
        ! Calculate size ...
        tmp = REAL(n1 * STORAGE_SIZE(arr, kind = INT64) / 8_INT64, kind = REAL64) / 1024.0e0_REAL64
        units = "KiB"
