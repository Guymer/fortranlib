!> @brief This subroutine allocates a 6D INT16 integer array.
!>
!> @param[inout] arr The 6D INT16 integer array to be allocated
!>
...
SUBROUTINE sub_allocate_6D_INT16_integer_array(arr, name, n1, n2, n3, n4, n5, n6, debug)
    ...
    IF(debug)THEN
        ! Calculate size ...
        tmp = REAL(n1 * n2 * n3 * n4 * n5 * n6 * STORAGE_SIZE(arr, kind = INT64) / 8_INT64, kind = REAL64) / 1024.0e0_REAL64
        units = "KiB"
    ...
    ! Allocate array ...
    ALLOCATE(arr(n1, n2, n3, n4, n5, n6), stat = errnum, errmsg = errmsg)
    IF(errnum /= 0_INT32)THEN
        WRITE(fmt = '("ERROR: Failed to ALLOCATE() """, a, """. ERRMSG = ", a, ". ERRNUM = ", i3, ".")', unit = ERROR_UNIT) name, TRIM(errmsg), errnum
        FLUSH(unit = ERROR_UNIT)
        STOP
    END IF
END SUBROUTINE sub_allocate_6D_INT16_integer_array
