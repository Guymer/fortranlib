!> @brief This subroutine saves a 2D INT8 integer array to a PPM file.
!>
!> @param[in] arr The 2D INT8 integer array to be saved to a PPM file
!>
...
SUBROUTINE sub_save_2D_INT8_integer_array_as_PPM(arr, fname, cm)
    USE ISO_FORTRAN_ENV
    ...
    INTEGER(kind = INT8), DIMENSION(:, :), INTENT(in)                           :: arr
    ...
    ! Check input ...
    IF(TRIM(cm) /= "fire" .AND. TRIM(cm) /= "jet" .AND. TRIM(cm) /= "r2g" .AND. TRIM(cm) /= "r2o2g")THEN
    ...
    ! Clean up ...
    DEALLOCATE(img)
END SUBROUTINE sub_save_2D_INT8_integer_array_as_PPM
