SUBROUTINE sub_shrink_2D_INT8_logical_array_to_2D_REAL64_real_array(nx, ny, arr, shrinkScale, shrunkenArr)
            ...
            !$omp schedule(dynamic)
                ...
                DO ix = 1_INT64, nx
                    ...
                    DO iy = 1_INT64, ny
                        ! Find the value ...
                        IF(arr(ix, iy))THEN
                            shrunkenArr(ix, iy) = 1.0e0_REAL64
                        ELSE
                            shrunkenArr(ix, iy) = 0.0e0_REAL64
                        END IF
                    END DO
                END DO
            !$omp end do
        ...
        !$omp schedule(dynamic)
            ...
            DO ix = 1_INT64, nx / shrinkScale
                ...
                DO iy = 1_INT64, ny / shrinkScale
                    ...
                    shrunkenArr(ix, iy) = REAL(COUNT(arr(ixlo:ixhi, iylo:iyhi), kind = INT64), kind = REAL64) / REAL(shrinkScale * shrinkScale, kind = REAL64)
                END DO
            END DO
        !$omp end do
    !$omp end parallel
END SUBROUTINE sub_shrink_2D_INT8_logical_array_to_2D_REAL64_real_array
