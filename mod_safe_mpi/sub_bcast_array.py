    ...
    for typ in sorted(data.keys()):
        ...
        for knd1, knd2 in zip(data[typ][0], data[typ][1], strict = True):
            ...
            for dim in range(7):
                ...
                src = (
                    f"SUBROUTINE sub_bcast_{dim + 1:d}D_{knd1}_{typ}_array(buff, root, comm)\n"
                    f"    ! This subroutine broadcasts a {dim + 1:d}D {knd1} {typ} array.\n"
                    "\n"
                    "    ! Import standard modules ...\n"
                    "    USE ISO_C_BINDING\n"
                    "    USE ISO_FORTRAN_ENV\n"
                    ...
                    f"    {lhs1:76s}:: buff ! The {dim + 1:d}D {knd1} {typ} array to be broadcast.\n"
                    ...
                    f"        CALL MPI_BCAST(buff_flat(i), parcel, {knd2}, root, comm, errnum)\n"
                    ...
                    "    ! Destroy pointer ...\n"
                    "    NULLIFY(buff_flat)\n"
                    f"END SUBROUTINE sub_bcast_{dim + 1:d}D_{knd1}_{typ}_array\n"
                )

                # Save source ...
                with open(f"sub_bcast_array/sub_bcast_{dim + 1:d}D_{knd1}_{typ}_array.f90", mode = "wt", encoding = "utf-8") as fObj:
                    fObj.write(src)

    # Open output file ...
    with open("sub_bcast_array.tmp", mode = "wt", encoding = "utf-8") as fObj:
